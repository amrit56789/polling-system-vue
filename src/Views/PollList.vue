...
<div class="max-w-4xl mx-auto py-6">
    ...
    <div v-for="(poll, index) in polls" :key="poll.id" class="mt-4 bg-gradient-to-r from-blue-100 via-blue-200 to-blue-100 rounded-lg shadow-lg px-5 py-4 hover:shadow-xl transition-shadow duration-300">
        <h3 class="text-2xl font-bold text-black-900 text-left p-4">{{ index + 1 }}. {{ poll.title }}</h3>
        ...
        <div v-if="user?.roleId === ADMIN_ROLE_ID" class="flex mt-4 justify-end">
            ...
            <button @click="showResults(poll.id)" class="text-white font-semibold rounded-lg transition-colors duration-200 flex items-center">
                <Icon icon="mdi:chart-bar" class="w-12 h-5 text-blue-500" />
            </button>
            <button @click="deletePoll(poll.id)" class="text-white font-semibold rounded-lg transition-colors duration-200 flex items-center">
                <Icon icon="mdi:delete" class="w-10 h-5 text-red-500" />
            </button>
        </div>
    </div>
    <div v-if="!isLoading && polls.length === 0" class="text-center text-gray-700">No data found</div>
